...
void blk_mq_end_request(struct request *rq, blk_status_t error)
{
	...
	if(rq->bio && rq->bio->xrp_enabled){
		...
		fd = rq->bio->xrp_cur_fd;
		ret = get_inode_from_xrp_fd_info_array(rq->bio->xrp_fd_info_arr, rq->bio->xrp_fd_count, fd, &inode);
		if (ret < 0) {
		...
		if (rq->bio->xrp_count > 1
		    && inode->i_op == &ext4_file_inode_operations) {
			file_offset = rq->bio->xrp_file_offset;
			data_len = 512;
		...
		ebpf_context.data = page_address(bio_page(rq->bio));
		ebpf_context.scratch = page_address(rq->bio->xrp_scratch_page);
		ebpf_context.cur_addr = rq->bio->xrp_file_offset;
		...
		if (inode->i_op == &ext4_file_inode_operations) {
			...
			if (!mapping.exist || mapping.offset != file_offset || mapping.len < data_len || mapping.address & 0x1ff) {
				...
				rq->bio->xrp_extent_version = mapping.version;
		...
		if (512 < blk_rq_bytes(rq)) {
			...
			if (rq->bio->xrp_original_bi_io_vec == NULL) {
				// save original values
				rq->bio->xrp_original_bi_io_vec = rq->bio->bi_io_vec;
				rq->bio->xrp_original_bi_vcount = rq->bio->bi_vcnt;
				rq->bio->xrp_original_bi_max_vecs = rq->bio->bi_max_vecs;
				rq->bio->xrp_original_bi_iter = rq->bio->bi_iter;

				// set new values, only needs to be done once
				rq->bio->bi_vcnt = 1;
				rq->bio->bi_max_vecs = 1;
				rq->bio->bi_io_vec = &rq->bio->xrp_bio_vec;
			}

			// set struct bio_vec values
			rq->bio->bi_io_vec->bv_page = bvec->bv_page;
			rq->bio->bi_io_vec->bv_offset = bvec->bv_offset;
			rq->bio->bi_io_vec->bv_len = blk_rq_bytes(rq);
...
REQUEST_END:
	...
	if (rq->bio != NULL && rq->bio->xrp_original_bi_io_vec != NULL) {
		rq->bio->bi_io_vec = rq->bio->xrp_original_bi_io_vec;
		rq->bio->bi_vcnt = rq->bio->xrp_original_bi_vcount;
		rq->bio->bi_max_vecs = rq->bio->xrp_original_bi_max_vecs;
		rq->bio->bi_iter = rq->bio->xrp_original_bi_iter;

		rq->bio->xrp_original_bi_io_vec = NULL;
		rq->bio->xrp_original_bi_vcount = 0;
