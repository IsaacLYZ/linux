...
int (*driver_get_nvmeof_xrp_info) (
	bool *xrp_enabled, uint32_t inode_identifier, struct inode **xrp_inode,
	struct bpf_prog **xrp_prog) = NULL;
...
static void nvmet_bdev_execute_rw(struct nvmet_req *req)
{
	...
	if (req->cmd->rw.opcode == nvme_cmd_xrp_read) {
		...
		ret = driver_get_nvmeof_xrp_info(
			&xrp_enabled, xrp_cmd_config.inode_identifier,
			&xrp_inode, &xrp_prog);
		if (ret) {
			pr_warn(
